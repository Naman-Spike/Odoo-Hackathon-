  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-10">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-black text-zinc-900 tracking-tight font-sans">Compensation Management</h1>
          <p className="text-xs text-zinc-500 mt-0.5 font-medium">Configure salary bands, basic compensation, and deductions for personnel</p>
        </div>
      </div>
      ...
      {/* Table */}
      {loading ? (
        <div className="py-16 text-center text-xs font-mono text-zinc-400">Syncing payroll ledger...</div>
      ) : (
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-zinc-100">
              {filteredPayrolls.length === 0 ? (
                <tr>
                  <td colSpan={7} className="text-center py-10 text-zinc-400 font-mono">
                    ...
                    <tr key={p.id} className="hover:bg-zinc-50/60 transition-colors">
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-2.5">
                          <div className="w-7 h-7 rounded-lg bg-black text-white font-bold text-[9px] flex items-center justify-center">
                            {getInitials(empName, '')}
                          </div>
                          <div>
                      ...
                      <td className="px-4 py-3.5 font-mono text-zinc-600 text-[11px]">{p.user?.profile?.department || 'General'}</td>
                      <td className="px-4 py-3.5 text-right font-mono text-zinc-700">{formatCurrency(p.basicSalary)}</td>
                      <td className="px-4 py-3.5 text-right font-mono text-zinc-700">+{formatCurrency(p.allowances)}</td>
                      <td className="px-4 py-3.5 text-right font-mono text-zinc-500">-{formatCurrency(p.deductions)}</td>
                      <td className="px-4 py-3.5 text-right font-mono font-bold text-zinc-900">{formatCurrency(p.netSalary)}</td>
      ...
      <Modal isOpen={editModalOpen} onClose={() => setEditModalOpen(false)} title="Adjust Compensation Package" size="md">
        {selectedPayroll && (
          <div className="space-y-4">
            <div className="bg-zinc-50 p-3.5 rounded-xl border border-zinc-200 flex items-center justify-between font-mono">
            ...
            <div className="mt-4 border-t border-zinc-100 pt-4">
              <div className="flex justify-between items-center mb-4 p-3 bg-zinc-50 rounded-xl border border-zinc-200 text-xs font-mono">
                <span className="font-bold text-zinc-700">Projected Net Disbursal:</span>
                <span className="text-base font-extrabold text-zinc-900">{formatCurrency(previewNetSalary)}</span>
              </div>
